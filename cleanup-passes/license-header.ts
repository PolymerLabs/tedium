...
const roughPolymerHeader = polymerHeader.split(EOL).slice(1).join(EOL).replace(/\s/g, '').toLowerCase();
...
function findFirstComment(lines: string[], start: RegExp, end: RegExp): CommentLocation {
  return {
...
function licenseRoughlyEqual(lines: string[], start: number, end: number): boolean {
  // "fix" the date
  const firstLine = lines[start + 1].replace(/\d{4}/, '2016');
  const license = [firstLine, ...lines.slice(start + 2, end)].join(EOL).replace(/\s/g, '');
...
async function addLicenseHeader(element: ElementRepo): Promise<void> {
  const foundFiles = await findFiles('**/*.{css,html,js}', {cwd: element.dir});
  const modifiedFiles: string[] = [];
  for (let filename of foundFiles) {
    ...
    const licenseFn: (lines: string[]) => void = isHTML ? addLicenseHTML : addLicense;
    if (start === -1 || end === -1) {
      // missing any comment
      licenseFn(lines);
  ...
  if (modifiedFiles.length) {
    await makeCommit(element, modifiedFiles, '[skip ci] Add license headers');
  }
}

register({
  name: 'license',
  pass: addLicenseHeader,
  runsByDefault: true
});
